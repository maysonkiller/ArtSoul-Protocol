...
- [ ] **A5 — Indexer status drift.** Verify confirmation depth, persisted cursor, reorg sampling, restart behavior, and health output against deployed environment values. Treat the current healthy response as evidence to verify, not a substitute for a restart/recovery test.
