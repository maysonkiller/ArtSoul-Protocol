...
- [ ] **A7 — Profile lifecycle and action gating.** Verify that only the correct wallet and lifecycle state expose publish, auction, settlement, mint, resale, moderation, or admin actions. In progress via PR #126: full action inventory audited; legacy-artwork write-action visibility, resale submit re-checks, self-purchase rejection, and dormant profile predicate chain guards hardened with a 28-test role/lifecycle matrix (strict Base-only chain authorization and live submit-time accounts included). Acceptance requires merge plus post-deploy verification.
