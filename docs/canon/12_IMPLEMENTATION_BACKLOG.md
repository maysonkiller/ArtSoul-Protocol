...
- [ ] **A5 — Indexer status drift.** _(in progress)_ Verify confirmation depth, persisted cursor, reorg sampling, restart behavior, and health output against deployed environment values. PR #112 was deployed to the Hetzner PM2 indexer on 2026-07-17: startup reconciled Base Sepolia from depth 12 to 3 and resumed without resetting the persisted cursor. Completion remains blocked by verified production schema drift in `rollback_events_from_block(bigint,numeric)`: the function references absent legacy `indexed_auctions` and `indexed_bids` tables, causing rollback to fail when reorg/state-hash recovery is invoked. Make rollback schema-aware, add regression coverage, redeploy, and repeat the restart/recovery check before accepting A5.
