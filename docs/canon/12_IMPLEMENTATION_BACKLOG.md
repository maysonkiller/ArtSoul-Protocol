...
- [ ] **A9 — Infrastructure cost and alerting.** Observe the deployed RPC diet for at least seven days, set Alchemy and Supabase usage alerts where the funded plans support them, resolve the optional `failed_events` metric, and document health/restart checks. Repository health-check tooling, no-cost plan constraints, thresholds, manual cadence and the evidence template are defined in `docs/runbooks/A9_INFRA_COST_MONITORING.md`. A-15 was accepted on 2026-07-22 after PR #136 was deployed to Hetzner at merge commit `32b2d49`: `failed_events` is retired, `event_processing_registry` is the fail-closed source of truth, `/health` was healthy with zero failed/dead events, and authenticated Prometheus gauges independently reported zero. No new table or migration is required. PR #138 implements A-40 by removing the never-invoked webhook alert path and replacing its false scalar RPC health values with the event listener's maintained rolling window; production acceptance remains pending. A9 remains open for the seven-day cost evidence and the explicit metrics-credential follow-up.
