...
- [ ] **A1 — Security and migration verification.** Verify public-repository secret hygiene, RLS, server-only credentials, deployment configuration, and the documented migration path. Rotate any credential that was ever genuinely exposed; do not rotate masked examples. Repository hardening, production RLS, the historical testnet migration baseline, bucket guardrails, GitHub Secret Scanning/push protection, preview/production headers, desktop SIWE, and post-hardening signed media/metadata uploads plus public reads are verified. A1 remains open for the private credential attestation, one production mobile SIWE smoke test, and authenticated rejection of unsupported MIME and an artwork-size request greater than 50 MB.
...
- [ ] **A8 — Moderation and reporting MVP.** Complete the complaint-driven Report flow, notice-and-takedown form, valid-claim hide path, review queue, notifications, and auditable staff actions. The A8a access-security foundation was merged in PR #129 with its flag disabled and migration unapplied: 15-minute passkey step-up, two founder passkeys, one one-time auditable bootstrap grant, and Safe-only founder recovery. Production activation waits for the final domain/RP ID and a rehearsed recovery path. PR #130 merged the A8b public Report intake with its flag disabled and migration unapplied; the approved controlled-beta intake limit is five new reports per reporter wallet across a rolling 24-hour window. A-22/A8c is in implementation review: the dedicated Protocol Admin queue, lazy server-confirmed menu discovery, serialized staff decisions with a distinct resolved status and duplicate-pending reopen guard, transition-accurate restore events, append-only decision evidence and notification obligations are built behind a separate disabled flag. A8 remains incomplete until migrations, passkeys, the admin workflow and public intake pass the resource-gated production checklist. X/Discord handles are not authentication factors. Do not build Content-ID or self-hosted fingerprinting.
- [ ] **A9 — Infrastructure cost and alerting.** Observe the deployed RPC diet for at least seven days, set Alchemy and Supabase usage alerts where the funded plans support them, resolve the optional `failed_events` metric, and document health/restart checks. Repository health-check tooling, no-cost plan constraints, thresholds, manual cadence and the evidence template are defined in `docs/runbooks/A9_INFRA_COST_MONITORING.md`. A-15 was accepted on 2026-07-22 after PR #136 was deployed to Hetzner at merge commit `32b2d49`: `failed_events` is retired, `event_processing_registry` is the fail-closed source of truth, `/health` was healthy with zero failed/dead events, and authenticated Prometheus gauges independently reported zero. No new table or migration is required. A-40 was accepted on 2026-07-22 UTC after PR #138 was deployed to Hetzner at merge commit `711027b`: the dormant webhook path is removed, the monitor returned `ok=true`, PM2 kept the Base Sepolia indexer online, and `/health` reported a 137 ms rolling RPC latency average, zero RPC errors in the last minute, and none of the removed fake throughput fields. A-41 was accepted on 2026-07-22 UTC after PR #140 was deployed at merge commit `c1decb2`: the focused cancellation suite passed 7/7 on Hetzner, PM2 remained online, and production health stayed green with zero unresolved, failed/dead, or rolling RPC errors. A-42 was accepted on 2026-07-24 after PR #142 was deployed at merge commit `1c37061`: the explicit credential was present without disclosure, unauthenticated/authenticated `/metrics` returned 401/200, port 3001 was loopback-only, the focused suite passed 15/15, the monitor returned `ok=true`, and PM2 remained online and was saved only after acceptance passed. A-43 was accepted on 2026-07-26 after the committed lease implementation from PR #144 was deployed with the reproducible-install repair from PR #145: the focused suite passed 19/19 on Hetzner, the index cursor advanced, production health remained green with zero unresolved, failed/dead, or rolling RPC errors, the registry contained 90 completed Base Sepolia events and no other status, the stale-processing query returned zero rows after the lease/reaper window, and PM2 saved the accepted process state. A9 remains open only for the required seven consecutive days of Alchemy/Supabase cost evidence.
- [ ] **A10 — Controlled beta entry.** Publish the tester checklist, support path, issue template, known prototype deviations, and go/no-go review. Exit requires no open P1 issue. Entry materials are prepared in `docs/testnet/CONTROLLED_BETA_ENTRY.md`; acceptance remains open until every Phase A gate is evidenced and the recorded decision is GO.
