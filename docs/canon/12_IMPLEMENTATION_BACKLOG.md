...
- [ ] **A9 — Infrastructure cost and alerting.** Observe the deployed RPC diet for at least seven days, set Alchemy and Supabase usage alerts where the funded plans support them, resolve the optional `failed_events` metric, and document health/restart checks. Repository health-check tooling, no-cost plan constraints, thresholds, manual cadence and the evidence template are defined in `docs/runbooks/A9_INFRA_COST_MONITORING.md`. A-15 is implemented in PR #136: `failed_events` is retired as a never-created dead subsystem, `event_processing_registry` becomes the single source of truth for event-processing failures, historical ranges fail closed instead of skipping unapplied events, and `/health` counts failed/dead rows. No new table or migration is required. Deployment, seven-day evidence and A-15 post-merge verification remain open.
