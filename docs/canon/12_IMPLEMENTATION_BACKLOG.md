...
- [x] **A6 — Projection and provenance verification.** Verify indexed Creator, First Collector, Owner, auction, settlement, resale, moderation, and pending-projection states across every public surface. Completed by PR #120 (surface audit, projection-state fixtures, wallet-fallback fix), PR #122 (exact chain-scoped lookup, indexer-backed provenance timeline, moderation-safe endpoint), and PR #124 (First Collector/Owner facts on shared cards, Owned creator-buyback filter). The hidden-artwork unavailable-copy item moved to A8 moderation UX.
