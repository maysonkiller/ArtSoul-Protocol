...
Phase A stands at **56 done, 16 in progress, 5 planned** across A-01 to A-77. A-47 was reopened on 2026-08-23 by new founder iOS evidence that contradicted browser-only acceptance: artwork loading handed off through a dark body. Its first preview repair also failed cold iOS acceptance because `createRoot` still replaced the visible static tree; the revised branch hydrates that tree in place and has not been accepted yet. A-58 was reopened the same day because first visits to profile tabs flashed synthetic cards; its revised iOS preview was accepted, while Android acceptance is still outstanding. A-61 was reopened by a new iOS capture of the large profile avatar partially painted; the already-accepted header path is not implicated, and revised iOS and Android profile visits are still required. A-75 closed on 2026-08-24 after the PR #237 Vercel preview rendered the audio-detail brand visual from the root asset with an image response. A-76 closed on 2026-08-26 after the PR #239 Vercel preview gave artwork 31 one truthful expired state: `No bids` plus `End Expired Auction`, with no live badge or bid action. A-77 records the measured post-gallery media cost: cards started audio/video work before interaction, including repeated MP4 range requests; its stacked preview keeps card media dormant until Play while retaining metadata preload on the selected artwork detail. A-54 was reopened on 2026-08-24 after new desktop and iOS evidence showed an empty app, a blank themed shell and an invisible mobile skeleton before profile content. Its static-shell repair removed those empty frames but made the 3-4 second critical-path wait more visible; the current revision releases identity before the artwork feed instead of disguising that wait. A-64 moved to `in progress` on 2026-08-24 after production manifest evidence confirmed that every profile visit still statically imported the wallet dependency graph. Its preview implementation replaces that import with a post-paint loader while keeping the existing wallet API, and must not merge until connected desktop, Android and iOS acceptance covers restore, reconnect, SIWE, Base Sepolia switching and publish. A-50, A-53 and A-56 await a run on a device with a connected wallet; A-51, A-52, A-55 and A-60 were closed on 2026-08-21 by measurement in a browser. Do not re-open a `done` row without new
