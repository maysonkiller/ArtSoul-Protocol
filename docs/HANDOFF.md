...
| Row | Work | First step |
| --- | --- | --- |
| A-47 | Keep artwork loading continuous and shorten the exact-artwork path | PR #233 is merged after iOS preview and cross-platform browser acceptance. The 2026-08-28 audit found the exact projection still cold-started through the all-routes API graph; the focused branch gives the exact URL a physical dedicated Vercel function without changing cache or response semantics. Confirm that function boundary on preview, then re-measure cold desktop, iOS, Android and the legacy-video tail. Do not cache live auction state unconditionally. |
