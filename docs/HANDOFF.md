...
Transaction feedback: [A-33 evidence and local verification, 2026-09-06](testnet/A33_TRANSACTION_FEEDBACK_CHECK_2026-09-06.md). A-33 remains in progress; no signed transaction was performed for this repair.
...
Phase A stands at **59 done, 16 in progress, 6 planned** across A-01 to A-81. An audit on 2026-09-03 opened A-79, A-80 and A-81 from the four open pull requests: A-79 records that PR #242 and the merged A-54/A-61 repairs contradict each other and must not keep replacing one another, A-80 records the agent layer's wallet check against the deferred runtime in PR #238, and A-81 records that the Phase A counters in four documents are what makes every parallel branch read as conflicting. A-78 reopened on 2026-09-06: the active-first resolver can select a different artwork's auction; token/artwork resale inputs have the same ambiguity. Explicit input namespaces now replace guessing, with connected-wallet acceptance still pending. A-77 opened on 2026-08-27 after the founder finalized an auction nobody bid on and could not auction the artwork again: `endAuction` releases the work on chain, but `AuctionEnded` is the terminal event for a no-bid auction and no handler cleared `active_auction_id`, so twelve artworks read as permanently occupied while the profile refused a new auction twice over. The indexer and both profile predicates are fixed; the backfill for rows already stored wrong is founder-run and still outstanding. A-47 was reopened on 2026-08-23 by new founder iOS evidence that contradicted browser-only acceptance: artwork loading handed off through a dark body. Its first preview repair also failed cold iOS acceptance because `createRoot` still replaced the visible static tree; the revised branch hydrates that tree in place and has not been accepted yet. A-58 was reopened the same day because first visits to profile tabs flashed synthetic cards; its revised iOS preview was accepted, while Android acceptance is still outstanding. A-61 was reopened by a new iOS capture of the large profile avatar partially painted; the already-accepted header path is not implicated, and revised iOS and Android profile visits are still required. A-75 closed on 2026-08-24 after the PR #237 Vercel preview rendered the audio-detail brand visual from the root asset with an image response. A-76 closed on 2026-08-26 after the PR #239 Vercel preview gave artwork 31 one truthful expired state: `No bids` plus `End Expired Auction`, with no live badge or bid action. A-54 was reopened on 2026-08-24 after new desktop and iOS evidence showed an empty app, a blank themed shell and an invisible mobile skeleton before profile content. Its static-shell repair removed those empty frames but made the 3-4 second critical-path wait more visible; the current revision releases identity before the artwork feed instead of disguising that wait. A-50, A-53 and A-56 await a run on a device with a connected wallet; A-51, A-52, A-55 and A-60 were closed on 2026-08-21 by measurement in a browser. Do not re-open a `done` row without new
...
These ten rows need no founder decision and no new spend:

| Row | Work | First step |
| --- | --- | --- |
| A-47 | Keep artwork loading continuous and shorten the exact-artwork path | PR #233 is merged after iOS preview and cross-platform browser acceptance. The focused branch gives the exact URL a physical dedicated Vercel function without changing cache semantics and seeds the public Creator name/avatar before the first content frame, removing the temporary generated identity reported on 2026-08-30. Confirm both on preview, then re-measure cold desktop, iOS, Android and the legacy-video tail. Do not cache live auction state unconditionally. |
...
| A-61 | Commit the large profile avatar only after its frame is decoded | Keep the original-proportion source and stable shell, decode off-DOM, and reject late results from a previous profile. |
...
| A-54 | Cross-page perceived load | Start the narrow public profile read from the document head and commit identity before Genesis/gallery data. Preserve the A-58 compact gallery loading status and do not restore synthetic cards. Accept on desktop, iOS and Android. |
