# RG-01 Apex-Origin Smoke Checklist

Purpose: close the last open condition of `RESOURCE_GATED_WORK.md` RG-01, which
is an explicit precondition of RG-03 and therefore of the whole A8 moderation
activation.

Owners: two operators. No engineering change and no spend is required.
Estimated time: about 30 minutes per operator, run independently.

Everything below runs on the **apex origin only**: `https://artsoulprotocol.com`.
Never run these on `artsoul.vercel.app` or on a Vercel preview URL. The entire
point is to prove that WalletConnect sessions, SIWE signatures, browser storage
and OAuth returns all work on the final origin, because every one of those is
origin-scoped and a preview result proves nothing about production.

Record each row as `PASS` or `FAIL`. On any `FAIL`, stop that block, capture what
you saw, and report it. A partial run is not evidence.
