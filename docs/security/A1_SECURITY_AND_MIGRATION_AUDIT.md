...
### Production RLS verification status (in progress)

The production read-only RLS verification is **not yet complete**. Only the `storage.objects` policy output for the `artworks` bucket has been reviewed so far; it exposed the storage gap corrected by Phase 18.7c above. The remaining verification outputs from `sql/verification/phase_a_security_verification.sql` (public-schema classification, RLS enabled/forced, client write grants, internal-table grants, `SECURITY DEFINER` functions, and the migration ledger) still have to be captured and reviewed before Phase A RLS verification can be declared complete. Do not treat the storage finding as the whole verification.
...
6. Migration baseline ledger reconciliation for the live database.
7. Preview and production header smoke tests.
8. Desktop and mobile SIWE sign-in smoke tests, plus a signed-upload and public-read smoke test after 18.7c.
