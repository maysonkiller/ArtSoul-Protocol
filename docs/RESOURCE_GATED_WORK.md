...
| RG-02 | Project email for general, security, and copyright operations | C14 | The permanent domain is live; a monitored project mailbox and retention procedure are not yet verified. **Deferred by the founder on 2026-09-04 until the project is funded.** Current infrastructure runs at roughly 150-200 USD a month against no revenue and no raise, and a mailbox is a recurring cost with no free option that also carries a reply path: forwarding services receive but cannot answer as the project, which is the half this row actually needs. The interim mitigation below is unchanged and remains correct - private operational channels stay in use and no unmonitored mailbox is published anywhere. This defers **C14** and nothing else: RG-03 does not require it, so moderation activation and Phase A exit are unaffected | Use existing private operational contact channels; do not publish a mailbox that is not monitored | A monitored receive/reply path for general, security, and copyright operations plus its retention procedure are verified |
