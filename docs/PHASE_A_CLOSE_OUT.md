...
| **A-54** | Keep profile loading visible from the first parsed frame | Reopened by contradictory desktop and iOS evidence: the route shipped an empty app root and the mobile no-motion rule cancelled the skeleton reveal while its opacity remained zero. The repair paints the exact skeleton in HTML, lets the document reach visible shells before styles settle, and hydrates the skeleton in place |
