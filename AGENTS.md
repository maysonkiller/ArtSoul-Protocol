...
16. VISUAL/THEME (doc 16): strict Classic/Future separation — a Future color NEVER appears in Classic and vice versa. ALL theme colors live as variables in `unified-styles.css` under `.classic`/`.future`; NEVER hardcode theme hex in pages or JS, use `var(--c-*)`. Mobile/desktop differ in LAYOUT only (colors identical across devices). Classic MAY animate but only in Classic colors (Amended 2026-08-10; supersedes "Classic = no animation") — functional feedback only, never Future glow/neon; Future = animation. All animation must respect `prefers-reduced-motion`.
